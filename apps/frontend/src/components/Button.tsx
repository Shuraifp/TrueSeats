...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  ...
  variant?: 'primary' | 'secondary' | 'danger' | 'logout' | 'primary_golden';
}
...
const Button: React.FC<ButtonProps> = ({ children, variant = 'primary', className, ...props }) => {
  ...
  const variants = {
    ...
    primary_golden: "bg-yellow-600 border-yellow-700 bg-opacity-50 hover:bg-yellow-700 hover:bg-opacity-75 text-white shadow-md hover:shadow-lg transform hover:scale-105 hover:shadow-[0_0_8px_rgba(255,215,0,0.7)]",
